...
class VibrationsData:
    ...
    be computed by a Calculator or some other algorithm.
    Once :class:`VibrationsData` has been constructed, this class provides some
    common processing options; frequency calculation, mode animation, DOS etc.

    If the Atoms object is a periodic supercell, :class:`VibrationsData` may be
    ...
    If the Atoms object has :class:`~ase.constraints.FixAtoms` or
    :class:`~.ase.constraints.FixCartesian` constraints,
    these will be respected and the Hessian should be sized accordingly.

    Parameters
    ----------
    atoms : :class:`~ase.Atoms`
        Equilibrium geometry of vibrating system.
        This will be stored as a full copy.

    hessian : np.ndarray
        Second-derivative in energy with respect to Cartesian nuclear movements
        as an (N, 3, N, 3) array.

    indices : Sequence[int] | np.ndarray | None, default: None
        Indices of atoms which are included in Hessian.
        By default, it includes all freely moving atoms (i.e. not fixed ones).
        Leave at None if constraints should be determined automatically from
        the atoms object.
    ...
    def from_2d(
        cls,
        atoms: Atoms,
        hessian_2d: Sequence[Sequence[Real]] | np.ndarray,
        ...
        hessian : Sequence[Sequence[Real]] | np.ndarray
            Second-derivative in energy with respect to Cartesian nuclear
            movements as a (3N, 3N) array.

        indices : Sequence[int] | None, default: None
            Indices of (non-frozen) atoms included in Hessian.
    ...
    def indices_from_constraints(atoms: Atoms) -> list[int]:
        ...
        atoms object. :class:`VibrationsData` only supports:

        - :class:`~ase.constraints.FixCartesian`
        - :class:`~ase.constraints.FixAtoms`

        All others are neglected.
    ...
    @staticmethod
    def indices_from_mask(
        mask: Sequence[bool] | np.ndarray,
    ) -> list[int]:
        """Indices corresponding to boolean mask

        This is provided as a convenience for instantiating
        :class:`VibrationsData` with a boolean mask.
        For example, if the Hessian data includes only the H
    ...
    def _check_dimensions(
        ...
        atoms : :class:`~ase.Atoms`
        hessian : np.ndarray
            Proposed Hessian array.
        indices : np.ndarray | Sequence[int]
            Indices of atoms used in Hessian.
        ...
        ------
        ValueError
            If Hessian dimensions are not (N, 3, N, 3).
    ...
    def tabulate(self, im_tol: float = 1e-8) -> str:
        ...
        ----------
        im_tol : float
            Tolerance for imaginary frequency in eV.
            If frequency has a larger imaginary component than im_tol,
            the imaginary component is shown in the summary table.
    ...
    def iter_animated_mode(
        self,
        mode_index: int,
        temperature: float = units.kB * 300.0,
        frames: int = 30,
    ) -> Iterator[Atoms]:
        ...
        ----------
        mode_index : int
            Selection of mode to animate.
        temperature : float
            Temperature in energy units - use ``units.kB * T_IN_KELVIN``
        frames : int
            Number of image frames in animation.
    ...
    def show_as_force(
        ...
        scale : float
            Scale factor.
        show : bool
            If True, open the ASE GUI and show atoms.
    ...
    def write_jmol(
        ...
        ir_intensities: Sequence[float] | np.ndarray | None = None,
    ) -> None:
        ...
        ir_intensities :
            If available, IR intensities can be included in the header lines.
            This does not affect the visualisation but may be convenient when
    ...
    def _get_jmol_images(
        ...
        ----------
        atoms : :class:`~ase.Atoms`
            The base atoms object; all images have the same positions
        energies : np.ndarray
            Complex vibrational energies in eV.
        modes : np.ndarray
            Eigenvectors array corresponding to atoms and energies.
            This should cover the full set of atoms (i.e. modes =
